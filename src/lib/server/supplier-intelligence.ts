...
async function fetchSanctionsSummary(supplier: NormalizedSupplier): Promise<SanctionsSummary> {
  let sanctions: Array<{ name: string; country: string }> = [];

  try {
    const csvPath = path.join(process.cwd(), "data", "sanctions_list.csv");
    ...
    sanctions = csvText
      ...
      .filter(Boolean)
      .map((line) => {
        const [name, country] = line.split(",").map((c) => c.trim().replace(/^"|"$/g, ""));
        return { name: name ?? "", country: country ?? "" };
      })
      .filter((row) => row.name);
  } catch {
    // sanctions file missing — no match
  }

  const supplierNameLower = supplier.name.toLowerCase();
  const match = sanctions.find(
    (s) =>
      s.name.toLowerCase().includes(supplierNameLower) ||
      supplierNameLower.includes(s.name.toLowerCase()) ||
      s.country.toLowerCase() === supplier.country.toLowerCase()
  );
  ...
  return {
    score,
    summary: match
      ? `Potential sanctions match found for ${supplier.name} (matched: ${match.name}, ${match.country}).`
      : `No sanctions matches found for ${supplier.name} in ${supplier.country}.`,
    citations: match
      ? [asCitation({ title: "OFAC Sanctions Match", url: "https://www.treasury.gov/ofac", source: "OFAC", note: `Matched: ${match.name} in ${match.country}` })]
      : []
  };
}
...
export async function runSupplierIntelligence(rawBody: unknown) {
  ...
  modeUsed: mode,
  summary,
  suppliers: sortedAnalyses.map((s) => ({
