...
async function downloadPDF(entry: any) {
  ...
  const tableRows = suppliers.map((s: any) => [
...
export default function ReportsPage() {
  ...
  const handleGenerateAndDownload = async () => {
    if (history.length === 0) return alert("No scan data yet. Run a live scan first.");
    setGenerating("all");
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <Button onClick={handleGenerateAndDownload} disabled={generating === "all"}>
          ...
          {generating === "all" ? "Generating..." : "Generate & Download Latest"}
        </Button>
      </section>
