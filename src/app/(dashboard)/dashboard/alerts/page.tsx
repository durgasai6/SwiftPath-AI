...
// remove alerts from mock-data import entirely
...
useEffect(() => {
  fetch("/api/history")
    ...
    .then((data) => {
      ...
      // Generate alerts from scan history — critical suppliers become alerts
      const generated: AlertEntry[] = [];
      history.slice(0, 5).forEach((entry: any) => {
        (entry.suppliers ?? []).forEach((s: any, i: number) => {
          if (s.riskScore >= 60) {
            generated.push({
              id: `${entry.id}-${i}`,
              severity: s.riskScore >= 75 ? "critical" : "warning",
              riskType: "AI Analysis",
              supplierId: `s-${i}`,
              supplierName: s.supplierName,
              country: s.country ?? "",
              flag: "",
              headline: `Risk score ${s.riskScore}/100 detected`,
              description: s.recommendation ?? "Elevated risk detected by AI scan.",
              tags: ["AI Scan", s.country ?? ""],
